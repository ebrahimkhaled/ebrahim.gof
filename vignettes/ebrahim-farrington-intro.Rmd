...
A key property of the directed tests is that they gain power **without**
inflating the type I error rate. In a quick Monte Carlo (n = 500, 1000
replications, α = 0.05), the rejection rates are:

| Test | Size (null model) | Power: omitted quadratic | Power: wrong link |
|---|:---:|:---:|:---:|
...
Every test keeps its size near the nominal 0.05 (the directed tests are not
liberal), while DEF and the ensemble are clearly more powerful than HL and EF
against both a smooth (omitted-quadratic) and a link (complementary log-log)
misspecification.
