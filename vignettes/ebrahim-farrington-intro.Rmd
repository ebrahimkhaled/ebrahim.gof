...
The main function `ebrahim_farrington_test()` performs the goodness-of-fit test:
...
results <- data.frame(
  ...
  P_value = sapply(group_sizes, function(g) {
    ebrahim_farrington_test(y, predicted_probs, G = g)$p_value
  })
)
print(results)
```
...
simulate_power <- function(n, beta_quad = 0.1, n_sims = 100, G = 10) {
  ...
  for (i in 1:n_sims) {
    ...
    ef_test <- ebrahim_farrington_test(y, pred_probs, G = G)
...
result_grouped <- ebrahim_farrington_test(
  y_grouped,
